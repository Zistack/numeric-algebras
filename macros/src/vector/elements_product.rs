...
fn def_elements_product_traits_inner
(
	algebra_type: Path,
	struct_type: Path,
	scalar_type: Path,
...
{
	let elements_product_trait: Path =
		parse_quote! (numeric_algebras::traits::ElementsProduct);
	...
	(
		...
		(
			Self:
				std::clone::Clone
				+ numeric_algebras::traits::One <#scalar_type>
				+ numeric_algebras::traits::MulAssign <#scalar_type, #scalar_type>
		)
	);
...

	{
		...
		for (member_algebra_type, member_type) in &member_algebras
		{
			...
			(
				...
				(
					#member_algebra_type:
						#elements_product_trait <#member_type, Output = #scalar_type>
				)
			);
		}
		...
		quote!
		{
			...
			impl #impl_generics
			#elements_product_trait <#struct_type>
			for #algebra_type
			#where_clause
			{
				type Output = #scalar_type;

				fn elements_product (self, x: #struct_type) -> Self::Output
				{
					use numeric_algebras::traits::One;

					let mut acc = self . clone () . one ();
...

	{
		...
		for (member_algebra_type, member_type) in &member_algebras
		{
			...
			(
				...
				(
					#member_algebra_type:
						#elements_product_trait <&#lifetime_a #member_type, Output = #scalar_type>
				)
			);
		}
		...
		quote!
		{
			#[automatically_derived]
			impl #impl_generics
			#elements_product_trait <&#lifetime_a #struct_type>
			for #algebra_type
			#where_clause
			{
				type Output = #scalar_type;

				fn elements_product (self, x: &#lifetime_a #struct_type)
				-> Self::Output
				{
					use numeric_algebras::traits::One;
...
struct DefElementsProductTraits
{
	for_token: Token! [for],
	generics: Generics,
...
impl Parse for DefElementsProductTraits
{
...
impl ToTokens for DefElementsProductTraits
{
...
fn try_def_elements_product_traits_inner_impl (input: proc_macro::TokenStream)
-> Result <proc_macro2::TokenStream>
{
	let
	(
		(algebra_item, struct_item),
		DefElementsProductTraits
		{
			generics,
			struct_type,
			scalar_type,
		...
		((DeriveInput, ItemStruct), DefElementsProductTraits)
	...
	let tokens = def_elements_product_traits_inner
	(
		algebra_type,
		struct_type,
		scalar_type,
...
pub fn __def_elements_product_traits_inner_impl (input: proc_macro::TokenStream)
-> proc_macro::TokenStream
{
	try_def_elements_product_traits_inner_impl (input)
		. unwrap_or_else (Error::into_compile_error)
		. into ()
}

fn try_def_elements_product_traits_impl (input: proc_macro::TokenStream)
-> Result <proc_macro2::TokenStream>
{
	let def_elements_product_traits = parse (input)?;

	let DefElementsProductTraits
	{
		struct_type,
		algebra_type,
		..
	...
	let inner_macro_path =
		parse_quote! (numeric_algebras::derive::__def_elements_product_traits_inner);
...
pub fn def_elements_product_traits_impl (input: proc_macro::TokenStream)
-> proc_macro::TokenStream
{
	try_def_elements_product_traits_impl (input)
		. unwrap_or_else (Error::into_compile_error)
		. into ()
}

macro_rules! __def_elements_product_traits_macro
{
	() =>
	{
		#[doc (hidden)]
		#[proc_macro]
		pub fn __def_elements_product_traits_inner (input: proc_macro::TokenStream)
		-> proc_macro::TokenStream
		{
			crate::vector::__def_elements_product_traits_inner_impl (input)
		}

		#[proc_macro]
		pub fn def_elements_product_traits (input: proc_macro::TokenStream)
		-> proc_macro::TokenStream
		{
			crate::vector::def_elements_product_traits_impl (input)
		}
	}
}

pub (crate) use __def_elements_product_traits_macro as def_elements_product_traits_macro;
